...
bool CZHOBTMIND::SplitBuffer(char *strBuffer,bool isxml)
{
  ...
  if (isxml==true)
  {
    ...
    if (strlen(tmp)>0) snprintf(m_zhobtmind.u,10,"%d",(int)(atof(tmp)*10));

    GetXMLBuffer(strBuffer,"wd",tmp,10);
    if (strlen(tmp)>0) snprintf(m_zhobtmind.wd,10,"%d",(int)(atof(tmp)*10));
...
bool CZHOBTMIND::InsertTable()
{
  ...
  if (m_stmt.execute()!=0)
  {
    ...
    // 答：失败的原因主要有2，一是重复记录，二是数据内容非法
    // 2,如果失败了怎么办，程序是否需要继续？是否rollback？是否返回false？
    // 答：如果失败的原因是数据内容非法，记录日志后继续，如果是记录重复，不必记录日志，且继续if (m_stmt.m_cda.rc!=1062)
