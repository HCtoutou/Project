...
struct st_arg
{
  ...
  char ip[31];			// 服务端的ip地址
  int port;			// 服务端的端口
  int ptype;			// 文件上传成功后，文件的处理方式：1-删除文件，2-移动到备份目录
  char clientpath[301];		// 本地文件存放的目录
  char clientpathbak[301];	// 文件成功上传后，本底文件备份的目录，只有当ptype=2时有效
...
void _help()
{
  ...
  printf("Sample:/project/tools1/bin/procctl 20 /project/tools1/bin/tcpputfiles /log/idc/tcpputfiles_surfdata.log \"<ip>192.168.92.128</ip><port>5005</port><ptype>1</ptype><clientpath>/tmp/tcp/surfdata1</clientpath><clientpathbak>/tmp/tcp/surfdata1bak</clientpathbak><andchild>true</andchild><matchname>*.XML,*.CSV</matchname><srvpath>/tmp/tcp/surfdata2</srvpath><timetvl>10</timetvl><timeout>50</timeout><pname>tcpputfiles_surfdata</pname>\"\n");
  printf("       /project/tools1/bin/procctl 20 /project/tools1/bin/tcpputfiles /log/idc/tcpputfiles_surfdata.log \"<ip>192.168.92.128</ip><port>5005</port><ptype>2</ptype><clientpath>/tmp/tcp/surfdata1</clientpath><clientpathbak>/tmp/tcp/surfdata1bak</clientpathbak><andchild>true</andchild><matchname>*.XML,*.CSV</matchname><srvpath>/tmp/tcp/surfdata2</srvpath><timetvl>10</timetvl><timeout>50</timeout><pname>tcpputfiles_surfdata</pname>\"\n\n\n");
  ...
  printf("timetvl	扫描本地文件目录的时间间隔，单位：秒，取值在1-30之间。\n");
  printf("timeout	本程序的超时时间，单位：秒，视文件大小和网络带宽而定，建议设置50以上。\n");
  printf("pname		进程名，尽可能采用易懂的，与其他进程不同的名称，方便排查故障。\n\n");
...
bool _tcpputfiles()
{
  ...
  while (true)
  {
    ...
    if (Dir.ReadDir()==false){logfile.Write("Dir.ReadDir() failed.\n"); break;}
