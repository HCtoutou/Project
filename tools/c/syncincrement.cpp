...
void _help()
{
  ...
  printf("where		同步数据的条件，填充在select remotekeycol from remotetname where remotekeycol>:1之后，注意不要加where关键字。\n");
...
bool _syncincrement(bool &bcontinue)
{
  ...
  char remkeyvalue[51];		// 从远程表查到的需要同步记录key字段值
  ...
  bindstr[strlen(bindstr)-1]=0;	// 最后一个逗号删掉
