...
struct st_arg
{
  ...
  char connstr1[101];	// 已抽取数据的递增字段最大值存放的数据库连接参数
} starg;
...
int main(int argc,char *argv[])
{
  ...
  // 注意，在调试程序的时候，可以启用类似以下的代码，防止程序超时。
...
void _help()
{
  ...
  printf("selectsql	从数据源数据库抽取数据的SQL语句，注意：时间函数的百分号需要四个，显示出来才有两个，被prepare之后只剩一个。\n");
  ...
  printf("starttime	程序运行的时间区间，例如02，13表示：如果程序启动时，踏中02时和13时	则运行，其它时间不运行。"\
...
bool _dminingmysql()
{
  ...
  while (true)
  {
    ...
    // 读取结果集之后在打开文件，防止没有结果集但是还打开了文件，这样就会生成一个空文件
...
bool instarttime()
{
  // 程序运行的时间区间，例如02，13表示：如果程序启动时，踏中02和13时则运行1，其它时间不运行
...
bool writeincfile()
{
  ...
  // 如果打开starg.incfilename文件失败
  // 也可能是文件丢了，那也办法，只能重新抽取
