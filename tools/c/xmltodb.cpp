...
// 从vxxmltotable容器中查找xmlfilename的入库参数，存放在stxmltotable结构体中
bool findxmltotable(char *xmlfilename);

// 调用处理xml文件的子函数，返回值：0-成功，其它都是失败
...
void _help()
{
  ...
  printf("xmlpatherr	入库失败文件的存放目录。\n");
  printf("timetvl	本程序的时间间隔，单位：秒，视业务需求而定，2-30秒之间。\n");
  printf("timeout	本程序的超时时间，单位：秒，视xml大小而定，建议设置30秒以上。\n");
...
bool _xmltoarg(char *strxmlbuffer)
{
  ...
  if (starg.timeout==0) { logfile.Write("pname is null.\n"); return false; }
...
bool _xmltodb()
{
  ...
  // 加载入库参数的计时器，初始化为50是为了在第一次进入循环的时候就加载参数。
  int counter=50;
  ...
  while (true)
  {
    ...
    // 打开starg.xmlpath目录,为了保证先生成的数据先入库，打开目录的时候，应该按文件名排序。
    ...
    while (true)
    {
      ...
      // 打开xml文件错误，这种情况一般不会发生，如果真发生了，程序将退出。
      if (iret==3)
      {
      ...
      if (iret==6)
      {
        logfile.WriteEx("failed，执行sql失败。\n");
        return false;
      }
    }

    // 如果刚才这次扫描到了有文件，表示不空闲，可能不断地有文件生成，就不sleep了。
...
int _xmltodb(char *fullfilename,char *filename)
{
  ...
  if (findxmltotable(filename)==false) return 1;	// 未找到配置参数
  ...
  while (true)
  {
    ...
    if (stmtins.execute()!=0)
    {
      ...
      {
        // 如果insert失败，记录出错的行和错误内容，函数不返回，继续处理数据
        logfile.Write("%s",strBuffer);
...
bool loadxmltotable()
{
  ...
  while (true)
  {
    ...
    GetXMLBuffer(strBuffer,"execsql",stxmltotable.execsql,300);	// 处理xml文件执行，执行的SQL语句
...
void CTABCOLS::initdata()  // 成员变量初始化函数
{
...
bool CTABCOLS::allcols(connection *conn,char *tablename)
{
  ...
  while (true)
  {
    ...
    // 如果业务有需要，可以修改以上的代码，增加对更多数据类型的支持。
    ...
    // 如果字段类型是date，把字段设置为19.yyyy-mm-dd hh:mi:ss
...
void crtsql()
{
  memset(strinsertsql,0,sizeof(strinsertsql));	// 插入表的SQL
  memset(strupdatesql,0,sizeof(strupdatesql));	// 更新表的SQL

  // 生成插入表的SQL语句。insert into 表名(%s) values(%s)
  char strinsertp1[30001];	// insert语句的字段列表
  char strinsertp2[30001];	// insert语句values后的内容
  ...
  for (int ii=0;ii<TABCOLS.m_vallcols.size();ii++)
  {
    ...
    if (strcmp(TABCOLS.m_vallcols[ii].datatype,"date")!=0)
      SNPRINTF(strtemp,101,sizeof(strtemp),":%d",colseq);
    else
      SNPRINTF(strtemp,101,sizeof(strtemp),"str_to_date(:%d,'%%%%Y%%%%m%%%%d%%%%H%%%%i%%%%s')",colseq);
  ...
  // 更新TABCOLS.m_vallcols中的pksql字段，在拼接update语句的时候要用到它。
...
void preparesql()
{
  ...
  for (int ii=0;ii<TABCOLS.m_vallcols.size();ii++)
  {
    ...
    if (TABCOLS.m_vallcols[ii].pkseq!=0) continue;
    // 注意注意：strcolvalue数组的使用不是连续的，是和TABCOLS.m_vallcols的下标一一对应的
