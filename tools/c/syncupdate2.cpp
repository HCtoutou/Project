...
void _help()
{
  ...
  printf("charset	数据库的字符集，这个参数要与远程数据库保持一致，否则会出现乱码的情况。\n");
  ...
  printf("maxcount	每批执行一次同步操作的记录数，不能超过MAXPARAMS宏(在_mysql.h中定义)，当syncupdate==2时有效。\n");
  printf("timeout	本程序的超时时间，单位：秒，视数据量的大小而定，建议设置30以上。\n");
  printf("pname		本程序运行时的进程名，尽可能采用易懂的，与其他进程不同的名称，方便排查故障。\n\n");
  printf("注意：1）remotekeycol和localkeycol字段的选取非常重要，如果用了myslq的自增字段，那么在远程表中数据生成后自增字段的值不可改变，否则会同步失败；2）当远程表中存在delete操作时，无法分批同步，因为远程表的记录被delete后就找不到了，无法从本地表中执行delete操作。\n\n\n");
...
bool _xmltoarg(char *strxmlbuffer)
{
  ...
  // 的返回值或运算结果。如果本参数为空，就用localtname表的字段列表填充。
  GetXMLBuffer(strxmlbuffer,"remotecols",starg.remotecols,1000);
  // if (strlen(starg.remotecols)==0) { logfile.Write("remotecols is null.\n"); return false; }

  // 本地表的字段列表，与remotecols不同，他必须是真实存在的字段，如果本参数为空，就用localtname表的字段列表填充
  ...
  if (starg.synctype==2)
  {
    // 远程表的连接参数，格式与localconnstr相同，当synctype==2时有效
